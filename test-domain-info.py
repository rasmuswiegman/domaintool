...
for domain in domains:
    ...
    except (dns.resolver.NXDOMAIN, dns.resolver.NoAnswer, dns.resolver.NoNameservers)        print(domain.strip(), 'Does Not have any MX Record')
    try:
        answerstxt = dns.resolver.resolve(domain.strip(), 'txt')
    except (dns.resolver.NXDOMAIN, dns.resolver.NoAnswer, dns.resolver.NoNameservers):
        print(domain.strip(), 'Does not have any TXT Record')
    try:
        answersdnssec = dns.resolver.resolve(domain.strip(), '
    else:
